...
function BookViewerDialog({ book, onClose, isOpen }) {
  ...
  useEffect(() => {
    if (book) {
      ...
      setIsLoading(true); setError(null); setTotalPages(null);
      fetch(`${API_BASE_URL}/pdf/info?pdf_file=${encodeURIComponent(book.pdf_dosyasi)}`)
        ...
        .then(data => setTotalPages(data.total_pages)).catch(() => setTotalPages('?'));
    }
  }, [book]);
  ...
  const handleImageError = () => { setError("Bu sayfa yüklenemedi."); setIsLoading(false); };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl w-[95vw] h-[95vh] flex flex-col p-0 gap-0">
        <DialogHeader className="p-4 border-b flex-shrink-0"><DialogTitle className="text-xl md:text-2xl text-slate-800 line-clamp-1">{book.kitap}</DialogTitle></DialogHeader>
        <div className="flex-grow flex justify-center items-center bg-slate-200 overflow-hidden relative">
          ...
          {error && <div className="text-center text-red-600 p-4"><ServerCrash className="mx-auto h-8 w-8 mb-2" />{error}</div>}
          {imageUrl && !error && (
            <Image key={imageUrl} src={imageUrl} alt={`Sayfa ${currentPage}`} onLoad={handleImageLoad} onError={handleImageError} fill style={{ objectFit: 'contain' }} className={`transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`} sizes="95vw" />
          )}
        </div>
            ...
            <Card className="max-w-3xl mx-auto shadow-xl shadow-slate-200/70 border-t-4 border-emerald-500 bg-white/90 backdrop-blur-lg rounded-2xl">
                <CardContent className="p-6 md:p-8">
                    <div className="space-y-4">
                        <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" /><Input placeholder="Konu, eser veya yazar adı..." value={query} onChange={(e) => setQuery(e.target.value)} className="text-lg h-14 pl-12 rounded-lg" /></div>
                        <Popover><PopoverTrigger asChild><Button variant="outline" className="w-full justify-between h-12 text-base text-slate-600 rounded-lg">{selectedAuthors.size > 0 ? `${selectedAuthors.size} yazar seçildi` : "Yazara Göre Filtrele"}<ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" /></Button></PopoverTrigger><PopoverContent className="w-[--radix-popover-trigger-width] p-0"><Command><CommandInput placeholder="Yazar ara..." /><CommandList><CommandEmpty>Yazar bulunamadı.</CommandEmpty><CommandGroup>
