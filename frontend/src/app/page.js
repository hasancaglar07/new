...
// ★★★ HATA ODAĞI: BookViewerDialog bileşeni dikkatle incelendi ★★★
function BookViewerDialog({ book, onClose, isOpen }) {
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            if (!isOpen || !totalPages || isLoading) return;
    ...
    }, [isOpen, currentPage, totalPages, isLoading]);
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-none w-screen h-screen p-0 gap-0 flex flex-col bg-slate-800">
                <DialogHeader className="p-3 border-b border-slate-700 flex-shrink-0 flex-row items-center justify-between text-white bg-slate-900/50">
                    ...
                    <Button aria-label="Kapat" variant="ghost" size="icon" onClick={onClose} className="text-slate-300 hover:text-white hover:bg-slate-700">
                        <X className="h-6 w-6"/>
                    </Button>
                </DialogHeader>
                <div className="flex-grow w-full h-full flex justify-center items-center overflow-hidden relative">
                    ...
                    {/* Bu bölümdeki render prop kullanımı kritik. Fonksiyonun JSX döndürdüğünden emin olunmalı. */}
                    {imageUrl && !error && (
                                ...
                                <>
                                    ...
                                    <TransformComponent wrapperClass="!w-full !h-full" contentClass="!w-full !h-full">
                                        <Image
                                            ...
                                            onError={() => { setError("Bu sayfa yüklenemedi."); setIsLoading(false); }}
                                            fill
                ...
                <DialogFooter className="flex-row justify-between items-center p-3 bg-slate-900/50 border-t border-slate-700 flex-shrink-0 text-white backdrop-blur-sm">
                    ...
                    <div className="flex items-center gap-2">
                        <Button aria-label="Önceki sayfa" onClick={() => setCurrentPage(p => p > 1 ? p - 1 : p)} disabled={currentPage <= 1 || isLoading} className="bg-slate-700 hover:bg-slate-600">
                        ...
                        <Button aria-label="Sonraki sayfa" onClick={() => setCurrentPage(p => totalPages && p < totalPages ? p + 1 : p)} disabled={!totalPages || currentPage >= totalPages || isLoading} className="bg-slate-700 hover:bg-slate-600">
...
// Diğer bileşenleriniz (ArticleViewerDialog, ArticleResultCard vs.) doğru görünüyor.
// ... Diğer tüm yardımcı bileşenleriniz buraya gelecek ...
...
export default function HomePage() {
    ...
    return (
      <div className="bg-slate-50 min-h-screen w-full font-sans">
               ...
               hasResults && (
                <motion.div key="results" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.02 } } }}>
                  <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 h-16 rounded-xl p-2 bg-slate-200/80">
                      <TabsTrigger value="kitaplar" disabled={allResults.books.length === 0} className="data-[state=active]:bg-white data-[state=active]:shadow-md data-[state=active]:text-emerald-700 text-sm md:text-base gap-2 rounded-lg font-semibold"><BookOpen /> Kitaplar <Pill className="bg-emerald-100 text-emerald-800">{allResults.books.length}</Pill></TabsTrigger>
                      <TabsTrigger value="makaleler" disabled={allResults.articles.length === 0} className="data-[state=active]:bg-white data-[state=active]:shadow-md data-[state=active]:text-orange-700 text-sm md:text-base gap-2 rounded-lg font-semibold"><Newspaper /> Makaleler <Pill className="bg-orange-100 text-orange-800">{allResults.articles.length}</Pill></TabsTrigger>
                      <TabsTrigger value="videolar" disabled={allResults.videos.length === 0} className="data-[state=active]:bg-white data-[state=active]:shadow-md data-[state=active]:text-sky-700 text-sm md:text-base gap-2 rounded-lg font-semibold"><Video /> Videolar <Pill className="bg-sky-100 text-sky-800">{allResults.videos.length}</Pill></TabsTrigger>
                      <TabsTrigger value="analizler" disabled={allResults.analyses.length === 0} className="data-[state=active]:bg-white data-[state=active]:shadow-md data-[state=active]:text-violet-700 text-sm md:text-base gap-2 rounded-lg font-semibold"><BotMessageSquare /> Analizler <Pill className="bg-violet-100 text-violet-800">{allResults.analyses.length}</Pill></TabsTrigger>
                    </TabsList>
                    
                    {/* ★★★ KESİN ÇÖZÜM: .map() döngülerinin doğru kullanımı ★★★ */}
        ...
        {/* BookViewerDialog'un çağrıldığı yer */}
