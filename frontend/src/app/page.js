...
function Highlight({ text, query }) {
    ...
    const highlightedHtml = parts.map((part, i) => 
        part.toLowerCase() === query.toLowerCase() 
        ? `<mark class="bg-lime-200 text-lime-900 px-1 rounded-md">${part.replace(/<b\b[^>]*>|<\/b>/gi, "")}</mark>` 
        : part
    ).join('');
...
function PagePreview({ pdfFile, pageNum }) {
    ...
    useEffect(() => {
        setIsLoading(true);
        setError(null);
        // --- DÜZELTME: Dosya adını URL için kodluyoruz ---
        const imageUrl = `${API_BASE_URL}/pdf/page_image?pdf_file=${encodeURIComponent(pdfFile)}&page_num=${pageNum}`;
        let objectUrl = null;

        fetch(imageUrl)
            .then(res => {
                if (!res.ok) return Promise.reject(new Error("Resim yüklenemedi."));
                return res.blob();
            })
            .then(blob => {
...
// --- YENİ: Kitap Okuma Dialog Bileşeni (Geliştirilmiş Hali) ---
function BookViewerDialog({ book, onClose, isOpen }) {
  ...
  useEffect(() => {
    if (book) {
      ...
      // --- YENİ: Toplam sayfa sayısını en başta çekiyoruz ---
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl w-[95vw] h-[95vh] flex flex-col p-0 gap-0">
        <DialogHeader className="p-4 border-b flex-shrink-0">
          <DialogTitle className="text-xl md:text-2xl text-slate-800 line-clamp-1">{book.kitap}</DialogTitle>
        </DialogHeader>
        <div className="flex-grow flex justify-center items-center bg-slate-200 overflow-hidden relative">
          ...
          {imageUrl && !error && (
            <Image
              key={imageUrl} // Sayfa değiştiğinde Image'in yeniden render olmasını sağlar
              src={imageUrl}
        ...
        <DialogFooter className="flex-row justify-between items-center p-3 bg-slate-100 border-t flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
                <Button onClick={() => setCurrentPage(p => p > 1 ? p - 1 : p)} disabled={currentPage <= 1 || isLoading}><ArrowLeft className="h-4 w-4" /></Button>
                <Input type="number" value={currentPage} onChange={(e) => {const val = Number(e.target.value); if(val > 0 && val <= totalPages) setCurrentPage(val)}} className="w-20 text-center font-bold" disabled={isLoading || !totalPages} />
                <span className="font-semibold text-slate-600">/ {totalPages || '...'}</span>
                <Button onClick={() => setCurrentPage(p => totalPages && p < totalPages ? p + 1 : p)} disabled={!totalPages || currentPage >= totalPages || isLoading}><ArrowRight className="h-4 w-4" /></Button>
            </div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function ResultCard({ result, onReadClick, query, index }) {
  const cardVariants = {
    ...
    visible: { opacity: 1, y: 0, transition: { delay: index * 0.05, ease: "easeOut" } }
  };
  return (
        ...
        <CardFooter className="flex-col items-start p-0 bg-slate-50/70">
            <Accordion type="single" collapsible className="w-full px-6">
                <AccordionItem value="item-1" className="border-b-0">
                    ...
                    <AccordionContent><PagePreview pdfFile={result.pdf_dosyasi} pageNum={result.sayfa} /></AccordionContent>
                </AccordionItem>
            </Accordion>
...
// --- YENİ: Öneri Etiketleri Bileşeni ---
...
// --- YENİ: İskelet Yükleyici Bileşeni ---
...
// --- YENİ: Bilgi / Hata Durumu Bileşeni ---
...
// --- ANA SAYFA BİLEŞENİ (TÜM MANTIK BURADA) ---
export default function HomePage() {
  ...
  const performSearch = useCallback(async (currentQuery, currentAuthors) => {
    ...
    try {
        ...
        if (!bookRes.ok || !videoRes.ok) {
            throw new Error("Arama sunucusuna ulaşılamadı. Lütfen backend'in çalıştığından emin olun.");
        }
  ...
  // Debounce (gecikmeli arama) için useEffect
  useEffect(() => {
    const handler = setTimeout(() => {
        performSearch(query, Array.from(selectedAuthors));
    }, 500); // 500ms gecikme
    return () => clearTimeout(handler);
  }, [query, selectedAuthors, performSearch]);
  ...
  const handleAuthorChange = (author) => {
    ...
    newSet.has(author) ? newSet.delete(author) : newSet.add(author);
                ...
                <CardContent className="p-6 md:p-8">
                    <div className="space-y-4">
                        <div className="relative">
                            ...
                            <Input placeholder="Konu, eser veya yazar adı..." value={query} onChange={(e) => setQuery(e.target.value)} className="text-lg h-14 pl-12 rounded-lg" />
                        </div>
                        <Popover>
                            <PopoverTrigger asChild><Button variant="outline" className="w-full justify-between h-12 text-base text-slate-600 rounded-lg">{selectedAuthors.size > 0 ? `${selectedAuthors.size} yazar seçildi` : "Yazara Göre Filtrele"}<ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" /></Button></PopoverTrigger>
                            <PopoverContent className="w-[--radix-popover-trigger-width] p-0"><Command><CommandInput placeholder="Yazar ara..." /><CommandList><CommandEmpty>Yazar bulunamadı.</CommandEmpty><CommandGroup>
                                {authors.map((author) => (<CommandItem key={author} onSelect={() => handleAuthorChange(author)}><Checkbox checked={selectedAuthors.has(author)} className="mr-2" /><span>{author}</span></CommandItem>))}
                            </CommandGroup></CommandList></Command></PopoverContent>
                        </Popover>
                    </div>
                </CardContent>
            </Card>
            {!query.trim() && <SuggestionTags onSelect={(tag) => setQuery(tag)} />}
        </motion.div>
