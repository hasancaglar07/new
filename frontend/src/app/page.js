...
function SuggestionTags({ onSelect }) {
    ...
    return (
        <div className="flex flex-wrap justify-center items-center gap-2 md:gap-3">
            ...
            {suggestions.map(tag => (
                <Button key={tag} variant="outline" size="sm" className="rounded-full transition-all hover:bg-emerald-50 hover:border-emerald-300" onClick={() => onSelect(tag)}>
                    {tag}
                </Button>
            ))}
        </div>
                ...
                <CardContent className="p-6 md:p-8">
                    <div className="space-y-4">
                        <div className="relative">
                            ...
                            <Input placeholder="Konu, eser veya yazar adı..." value={query} onChange={(e) => setQuery(e.target.value)} className="text-lg h-14 pl-12 pr-12 rounded-lg" />
                            {isLoading && (
                                <div className="absolute right-4 top-1/2 -translate-y-1/2"><Loader2 className="h-5 w-5 text-slate-400 animate-spin" /></div>
                            )}
                        </div>
                        <div className="flex items-center gap-2">
                           ...
                           {selectedAuthors.size > 0 && (
                             <Button aria-label="Filtreleri temizle" variant="ghost" size="icon" className="h-12 w-12 shrink-0 text-slate-500 hover:text-slate-800" onClick={() => setSelectedAuthors(new Set())}>
                                <X className="h-5 w-5" />
                             </Button>
                           )}
                        </div>
